  ...
  methods: {
    ...
    setdriverAccess() {
      ...
      if(this.driver_id.length) {
        driverApi.setAccessdriver.call(this, {
          driver_id: this.driver_id.join(','),
          driver_accesss: this.checkeds.join(',')
        }, data => {
    ...
    /**
     * 表格列表触发CheckBox的事件
     * @param  {array} val 当前选中的司机信息数组，每个元素是司机信息对象
     */
     ...
     * 设置状态
     */
    onSetStatusdriver(driver, index, list) {
      this.$$api_driver_updatedriverStatus({
        id: driver.id
      }, (data) => {
    ...
    /**
     * 删除司机事件
     * @param  {object || boolean} driver  当前司机信息对象或者为布尔值,为布尔值时，代表是批量删除
     * @param  {number} index 当前司机列表索引
     * @param  {array} list  当前司机列表数组
     */
    onDeletedriver(driver, index, list) {
      // console.dir(driver);
      let title = '你确定删除司机 ' + driver.drivername + ' 么?';
      if(driver === true) {
      ...
      this.$confirm(title, '删除司机', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.$$api_driver_deleteDriver({
          id: id
        }, (data) => {
          if(driver === true) {
            this.driver_list = this.driver_list.filter(function(item, idx) {
    ...
    /**
     * 修改司机
     * @param  {object} driver 当前司机信息对象
     */
    onEditdriver(driver) {
      if(driver && driver.id) {
        this.$router.push('/manage/driver/edit?id=' + driver.id);
      } else {
        this.$message({
          showClose: true,
          message: 'ID跑哪去了？',
    ...
    onSelectdriver(driver) {
      ...
      this.dialog.driver_info = driver;
    },
    ...
    getList({
      ...
      this.$$api_driver_selectDriver(data, (data) => {
        this.driver_list = data.list.data;
