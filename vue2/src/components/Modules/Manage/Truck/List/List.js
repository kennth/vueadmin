module.exports = {
  ...
  methods: {
    ...
    settruckAccess() {
      ...
      if(this.truck_id.length) {
        truckApi.setAccesstruck.call(this, {
          truck_id: this.truck_id.join(','),
          truck_accesss: this.checkeds.join(',')
        }, data => {
    ...
    /**
     * 删除车辆事件
     * @param  {object || boolean} truck  当前车辆信息对象或者为布尔值,为布尔值时，代表是批量删除
     * @param  {number} index 当前车辆列表索引
     * @param  {array} list  当前车辆列表数组
     */
    onDeletetruck(truck, index, list) {
      // console.dir(truck);
      let title = '你确定删除车辆 ' + truck.truckname + ' 么?';
      if(truck === true) {
      ...
      this.$confirm(title, '删除车辆', {
        ...
        this.$$api_truck_deleteTruck({
          id: id
        }, (data) => {
          if(truck === true) {
            this.truck_list = this.truck_list.filter(function(item, idx) {
    ...
    /**
     * 修改车辆
     * @param  {object} truck 当前车辆信息对象
     */
    onEdittruck(truck) {
      if(truck && truck.id) {
        this.$router.push('/manage/truck/edit?id=' + truck.id);
      } else {
        this.$message({
          showClose: true,
          message: 'ID跑哪去了？',
     ...
     */
    onSelecttruck(truck) {
      this.dialog.truck_info.positioncode.lng = truck.longitude;
      this.dialog.truck_info.positioncode.lat = truck.latitude;
      this.dialog.truck_info.licenseplate = truck.licenseplate;
     ...
     * 设置状态
     */
    onSetStatusTruck(truck, index, list) {
      this.$$api_truck_saveTruck({
        id: truck.id
      }, (data) => {
        truck.status = truck.status == 1 ? 2 : 1;
      });
    },
    ...
    getList({
      ...
      this.$$api_truck_selectTruck(data, (data) => {
        this.truck_list = data.list.data;
